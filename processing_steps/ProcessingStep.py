...
class ProcessingStep(object):
    ...
    def __do_change__(self, df, row_num, column_name):
        ...
        """
        row = df.iloc[row_num]
        logging.debug(f"{self.ps_name} | {column_name} {row['date']} {row['time']}: {row[column_name]} -> {self.ERROR_VALUE}")
        changed_object = {
    ...
    def write_log(self,station):
      ...
      logging.debug(f"finished writing {len(self.changed)} entries into changelog for {log_filename} ...")
