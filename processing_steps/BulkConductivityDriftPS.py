    ...
    def process(self, df: pd.DataFrame, station: str):
        ...
        for row_num, row in df.iterrows():
            ...
            for column_lr in [self.ELEMENT_ORDER_LEFT, self.ELEMENT_ORDER_RIGHT]:
                ...
                [self.compare_and_prepare_to_change(column_lr, row_num, row, i) for i in range(len(column_lr))]

        # run do change for all to change values
        [self.__do_change__(df, row_num, column_name) for row_num, column_name in self.to_change]
        self.to_change = []
        # write logs
