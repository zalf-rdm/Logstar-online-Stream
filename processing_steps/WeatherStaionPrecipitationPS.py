...
class WeatherStaionPrecipitationPS(ProcessingStep):

  ps_name = "WeatherStaionPrecipitationPS"

  # value to fill if missmeasurement detected
  ERROR_VALUE = float("NaN")

  # apply to stations
  ALLOWED_STATIONS = ["weather_station_01", "weather_station_02", "weather_station_02_new"]
  ...
  def process(self, df: pd.DataFrame, station: str, argument: List = None):
    ...
    for i, row in df.iterrows():
      ...
      if row[self.COLUMN_NAME] == df.at[i-1,self.COLUMN_NAME] == df.at[i+1,self.COLUMN_NAME]:
        logging.debug(f"{self.ps_name} | {row['date']} {row['time']}: {row[self.COLUMN_NAME]} -> {self.ERROR_VALUE}")
        changed_object = {
