...
for group, _ in GROUPS.items():
    g_df = pd.DataFrame(columns=['dateTime', "patch_ID","parameter", "depth", "sensor_type"])
    # g_df["dateTime"] = pd.to_datetime(g_df["dateTime"])
    # g_df.set_index(f"dateTime") 
    GROUPS[group] = g_df

# files to ignore
file_blacklist = ["weather_station_02.csv", "weather_station_01.csv", "weather_station_02.2.csv"]
    ...
    for group, g_df in GROUPS.items():
        ...
        for group_column in group_columns:
            ...
            if "mobil" in station:
                ...
                filtered_df['patch_ID']=station.split("_")[0] + "_" + station.split("_")[1]
            else:
